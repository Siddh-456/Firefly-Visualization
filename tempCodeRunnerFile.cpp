...
struct Slider {
  ...
  void draw(Font font, Color col, const char *label = "Speed",
    ...
    float displayVal = Lerp(minV, maxV, value);
  ...
  float getInterval() {
    float multiplier = Lerp(1.0f, 10.0f, value);
    return 0.1f / multiplier; // 1x = 10 steps/sec, 10x = 100 steps/sec
  }
};
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    speedSlider.track = {CTRL_RECT.x + CTRL_RECT.width - 280.f,
                         ...
                         8.f};
    speedSlider.draw(fontSm, PAL.gold, "Speed", "%.1fx", 1.0f, 10.0f);
