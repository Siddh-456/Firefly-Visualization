...
struct FAEngine {
  ...
  Color col = WHITE;
  float prevBest = 0.f;  // for stagnation detection
  int stagnantCount = 0; // consecutive iters without improvement
  ...
  void step() {
    ...
    // Discrete Swap Mutation for Modified FA to escape local optima
    if (isModified) {
      float mutRate = (N <= 10) ? 0.2f : 0.35f; // higher rate for big boards
      int nSwaps = std::max(1, (N - 6) / 3); // more swaps for big boards (1 for
                                             // N≤8, 2 for N≤11, 3 for N≤14, …)
      for (int i = 0; i < POP; i++) {
        if ((float)GetRandomValue(0, 100) / 100.f < mutRate) {
          for (int s = 0; s < nSwaps; s++) {
            int r1 = GetRandomValue(0, N - 1);
            int r2 = GetRandomValue(0, N - 1);
    ...
    if (stagnantCount >= 40 && best < (float)MAX_FIT) {
      stagnantCount = 0;
      // Population is sorted descending — bottom half = worst individuals
      std::vector<int> base(N);
      std::iota(base.begin(), base.end(), 0);
...
void DrawComparisonTable(Rectangle bounds, const FAEngine &orig,
  ...
  // Header
  DrawTextEx(font, "Metric", {xL, y}, 15, 1, PAL.dimText);
  DrawTextEx(font, "Original", {xO, y}, 15, 1, PAL.orig);
  DrawTextEx(font, "Modified", {xM, y}, 15, 1, PAL.mod);
  y += rowH;
  ...
  for (auto &row : rows) {
    ...
    DrawTextEx(font, row.label, {xL, y + 4}, 15, 1, PAL.text);
    DrawTextEx(font, row.valO.c_str(), {xO, y + 4}, 15, 1, PAL.orig);
    DrawTextEx(font, row.valM.c_str(), {xM, y + 4}, 15, 1, PAL.mod);
    y += rowH;
...
int main() {
  ...
  StatsTicker ticker;
  Slider speedSlider;
  speedSlider.track = {1060, 840, 260, 8};
  ...
  Button btnMod = {{168, 795, 140, 44}, "Modified FA", PAL.mod};
  Button btnBoth = {{316, 795, 140, 44}, "Run Both", PAL.accent};
  Button btnReset = {{464, 795, 120, 44}, "Reset", PAL.conflict};
  Button btnStep = {{592, 795, 100, 44}, "Step", PAL.safe};
  Button btnDecN = {{700, 795, 44, 44}, "N-", PAL.dimText};
  Button btnIncN = {{752, 795, 44, 44}, "N+", PAL.dimText};
  ...
  while (!WindowShouldClose()) {
    ...
    btnCloseGraph.update(dt); // always update, not just when expanded
    ...
    if (btnDecN.pressed && N > 4) {
      N--;
      recalcParams();
      showO = showM = running = showCmpTable = false;
      origFA.done = modFA.done = true;
    }
    if (btnIncN.pressed && N < 32) {
      N++;
      recalcParams();
      showO = showM = running = showCmpTable = false;
      origFA.done = modFA.done = true;
    ...
    DrawTextEx(fontBold, "FIREFLY ALGORITHM VISUALIZER", {20, 12}, 24, 1,
               PAL.gold);
    DrawTextEx(fontSm, TextFormat("N=%d Queens | FA Optimisation", N), {20, 36},
               14, 1, PAL.dimText);
    ...
    btnDecN.draw(fontBold);
    btnIncN.draw(fontBold);
    ...
    drawCheck(810, 800, "Conflict Lines", showConflicts);
    drawCheck(810, 820, "Trails", showTrails);
    drawCheck(940, 800, "Compare Table", showCmpTable);
    ...
    if (expandedT > 0.01f) {
      ...
      DrawRectangleRoundedLines({legBoxX, legBoxY, legBoxW, legBoxH}, 0.08f,
      ...
      DrawRectangleRounded({spX - 8, spY, spW + 8, spH}, 0.08f, 8,
                           ColorAlpha(PAL.bg, 0.6f));
      DrawRectangleRoundedLines({spX - 8, spY, spW + 8, spH}, 0.08f, 8, 0.5f,
                                  ColorAlpha(PAL.panelBord, 0.6f));
